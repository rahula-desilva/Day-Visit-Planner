              ...
              {showSuggestions && customLocation.trim().length > 0 && (
                <div className="absolute top-[110%] left-0 z-[100] w-full bg-white border border-gray-100 rounded-2xl shadow-2xl overflow-hidden">
                  {[
                    ...
                    .filter((city) =>
                      city.toLowerCase().includes(customLocation.toLowerCase())
                    )
                    .map((city) => (
                      <div
                        key={city}
                ...
                >
                  <div
                    className={`w-4 h-4 bg-white rounded-full absolute top-1 transition-all ${
                      includeLunch ? "left-7" : "left-1"
                    }`}
                  ></div>
                </div>
                <span className="ml-3 text-sm font-bold text-gray-600 group-hover:text-primary whitespace-nowrap">
                  Lunch Break
                </span>
              </label>
            </div>
          ...
          <div className="mt-12 flex items-center gap-3 overflow-x-auto px-2 pb-4 no-scrollbar">
            {categories.map((cat) => (
              <button
                ...
                className={`px-8 py-3 rounded-full font-bold whitespace-nowrap transition-all duration-200 active:scale-95 ${
                  activeCategory === cat
                    ? "bg-[#005ab7] text-white shadow-lg scale-105"
                    : "bg-gray-100 text-gray-600 hover:bg-[#005ab7] hover:text-white hover:scale-105 hover:shadow-md border border-transparent"
                }`}
              >
                {cat}
              </button>
        ...
        <section className="py-24 px-6 md:px-12 max-w-7xl mx-auto">
          <div className="flex flex-col md:flex-row items-start md:items-end justify-between mb-16 gap-6">
            <div>
              ...
              <h2 className="font-headline text-4xl md:text-5xl font-extrabold text-gray-900 mt-2">
                Iconic Sri Lankan Spots
              </h2>
            </div>
            <button
              onClick={onExplore}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10">
            {displayedPlaces.length > 0
              ? displayedPlaces.map((place) => (
                  <div
                    key={place.id}
                    className="group flex flex-col place-card relative"
                  >
                    <div className="relative h-80 overflow-hidden rounded-[2rem] bg-gray-100 shadow-xl shadow-gray-200/50">
                      <img
                        ...
                        src={
                          place.image_url ||
                          "https://images.unsplash.com/photo-1544620347-c4fd4a3d5962?auto=format&fit=crop&q=80"
                        }
                        alt={place.name}
                        onError={(e) => {
                          e.target.src =
                    ...
                    <div className="mt-8 flex flex-col h-full">
                      <div className="min-h-[56px] flex justify-between items-start mb-2 gap-2">
                        <h3 className="font-headline font-bold text-2xl text-gray-900 leading-tight">
                          {place.name}
                        </h3>
                        <span className="flex items-center text-sm font-bold bg-yellow-50 px-2 py-1 rounded-lg">
                          <span
                            className="material-symbols-outlined text-yellow-500 text-lg mr-1"
                            style={{ fontVariationSettings: "'FILL' 1" }}
                          >
                            star
                          </span>
                      ...
                      <div className="min-h-[100px] mb-6">
                        <p className="text-gray-900 text-base leading-relaxed font-semibold">
                          {place.description ||
                            "A breathtaking destination with stunning views and local charm."}
                        </p>
                      </div>
                          ...
                          <div className="relative z-30 text-left">
                            <button
                              onClick={(e) => {
                                e.stopPropagation();
                                setOpenTipId(
                                  openTipId === place.id ? null : place.id
                                );
                              }}
                              className={`tip-button flex items-center gap-3 text-xs font-black transition-all ${
                                openTipId === place.id
                                  ? "text-amber-700"
                            ...
                            >
                              <div
                                className={`w-9 h-9 rounded-full flex items-center justify-center border-2 transition-all ${
                                  openTipId === place.id
                                    ? "bg-amber-100 border-amber-600 shadow-lg scale-110"
                            ...
                            {openTipId === place.id && (
                              <div className="absolute bottom-full left-0 mb-4 p-5 bg-white border-2 border-amber-400 shadow-[0_20px_50px_rgba(0,0,0,0.2)] rounded-2xl animate-in zoom-in-95 fade-in duration-200 min-w-[260px] w-full max-w-[300px] z-50">
                                ...
                                <h4 className="text-[11px] font-black text-amber-600 uppercase tracking-[0.2em] mb-2 relative z-10">
                                  EXPERT ADVICE
                                </h4>
          ...
          <div className="max-w-7xl mx-auto px-6 md:px-12 grid grid-cols-1 lg:grid-cols-2 items-center gap-20 md:gap-32">
            ...
            <div className="lg:pl-12">
              ...
              <h2 className="font-headline text-5xl md:text-6xl font-extrabold text-gray-900 mt-4 mb-8 leading-[1.1]">
                Plan your trip in <br className="hidden md:block" /> seconds,
                not hours.
              </h2>
              <p className="text-gray-500 text-xl mb-12 leading-relaxed font-medium">
                Subscribe to get exclusive one-day itineraries and hidden gem
                alerts delivered to your inbox every weekend.
              </p>
