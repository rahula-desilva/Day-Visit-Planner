...
export default function SavedTripsView({ session, onOpenAuth }) {
  ...
  const handleDelete = async (e, tripId) => {
    e.stopPropagation(); // Prevent opening the trip details
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center py-20">
        <div className="animate-spin text-5xl mb-4 text-indigo-600">🌀</div>
        <p className="text-gray-500 font-medium font-sans">Fetching your trips...</p>
      </div>
    );
  }
      ...
      <div className="space-y-6 animate-fadeIn">
        <div className="flex items-center justify-between mb-4 flex-wrap gap-4">
          <button 
            ...
            className="flex items-center gap-2 text-indigo-600 font-bold hover:text-indigo-800 transition-all"
          >
            ← Back to History
          </button>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 h-[calc(100vh-280px)] min-h-[500px]">
          ...
          <div className="bg-white rounded-2xl shadow-xl overflow-y-auto p-6 border border-gray-100">
            <div className="relative border-l-4 border-indigo-500 pl-8 space-y-10 ml-4 py-4">
              {sortedPlaces.map((tp, idx) => (
                <div key={tp.id} className="relative">
                  <div 
                    ...
                    className="absolute -left-[45px] top-1 bg-indigo-500 text-white font-bold w-8 h-8 rounded-full border-4 border-white flex items-center justify-center shadow-md cursor-pointer hover:scale-110 active:scale-95 transition-all"
            ...
            >
              ...
              {sortedPlaces.map((tp) => (
                <Marker 
                  ...
                  icon={L.divIcon({
                    html: `<div class="flex items-center justify-center w-8 h-8 rounded-full border-2 border-white shadow-lg text-white font-bold bg-indigo-600">${tp.order_number}</div>`,
              ...
              {routeGeometry && <Polyline positions={routeGeometry} color="indigo" weight={5} opacity={0.6} />}
              <MapRefresher points={[
      ...
      {trips.length === 0 ? (
        <div className="bg-white rounded-3xl p-12 text-center shadow-xl border border-gray-100 mt-8">
          ...
          <button 
            ...
            className="bg-indigo-600 text-white px-8 py-3 rounded-xl font-bold hover:bg-indigo-700 active:scale-95 transition-all shadow-lg"
          >
            Start Planning
          </button>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {trips.map((trip) => (
            ...
            >
              <div className="bg-indigo-600 p-5 text-white flex justify-between items-center group-hover:bg-indigo-700 transition-colors">
                <span className="font-bold">🗓️ {new Date(trip.date).toLocaleDateString()}</span>
              ...
              <div className="p-6 flex-grow">
                <div className="space-y-3">
                  {trip.trip_places
                    ...
                    .slice(0, 4) // Show only first 4 stops in preview
                    .map((tp) => (
